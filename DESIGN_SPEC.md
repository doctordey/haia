# Haia — FX Trading Performance Analytics Platform
...
export const users = pgTable('users', {
  ...
  calendarStart: text('calendar_start').notNull().default('monday'), // "monday" or "sunday"
...
export const tradingAccounts = pgTable('trading_accounts', {
  id:         text('id').primaryKey().$defaultFn(() => createId()),
  userId:     text('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
  name:       text('name').notNull(),                     // User-given label, e.g. "Main Account"
  platform:   text('platform').notNull(),                 // "MT4" or "MT5"
  metaApiId:  text('meta_api_id').notNull().unique(),     // MetaApi account ID
  server:     text('server').notNull(),                   // Broker server name
  login:      text('login').notNull(),                    // MT4/MT5 login number
  broker:     text('broker'),
...
export const trades = pgTable('trades', {
  ...
  ticket:      text('ticket').notNull(),                  // MT4/MT5 ticket number
  ...
  profit:      real('profit').notNull(),                  // PNL in account currency
  pips:        real('pips'),                              // PNL in pips
  commission:  real('commission').notNull().default(0),
  swap:        real('swap').notNull().default(0),
  isOpen:      boolean('is_open').notNull().default(false),
  magicNumber: integer('magic_number'),                   // EA identifier
...
export const dailySnapshots = pgTable('daily_snapshots', {
  ...
  pnl:        real('pnl').notNull(),                      // Realized PNL for this day
  tradeCount: integer('trade_count').notNull(),
  winCount:   integer('win_count').notNull(),
  lossCount:  integer('loss_count').notNull(),
  volume:     real('volume').notNull(),                   // Total lots traded
...
export const accountStats = pgTable('account_stats', {
  ...
  avgTradeDuration:  integer('avg_trade_duration').notNull().default(0), // in minutes
...
export const flexCards = pgTable('flex_cards', {
  ...
  title:           text('title'),
  period:          text('period').notNull(),               // "1D", "7D", "30D", "90D", "1Y", "MAX", or ISO date range
  metric:          text('metric').notNull(),               // "pnl", "winrate", "profitfactor", "monthlyreturn", "sharpe", "pctgain", "pips", "calendar"
...
NEXTAUTH_URL=              # https://haia.up.railway.app
...
7. Custom domain: `haia.yourdomain.com` via Railway settings
...
- [ ] Project setup: Next.js + Tailwind + Drizzle + Railway
