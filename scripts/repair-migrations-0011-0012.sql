-- ─────────────────────────────────────────────────────────────────────────────
-- Manual repair: apply migrations 0011 + 0012 to a database stuck at 0010
-- (symptom: "Failed to create key" — the api_keys table is missing).
--
-- HOW TO RUN on Railway: the Postgres "Console" tab is a bash shell, NOT a SQL
-- prompt. Start psql first, then paste this:
--     psql "$DATABASE_URL"        (if that says empty, just run: psql)
--   …then paste everything below, then \q to exit.
-- Or in one shot from the shell:
--     psql "$DATABASE_URL" <<'SQL'
--     <paste this file>
--     SQL
--
-- Idempotent — safe to re-run. No DO blocks / no BEGIN-COMMIT, so it also works
-- one-statement-at-a-time in a naive query console. Assumes base migrations
-- 0000–0010 are already applied (users/trades/trading_accounts +
-- drizzle.__drizzle_migrations exist).
-- ─────────────────────────────────────────────────────────────────────────────
...
-- Foreign keys (drop-then-add instead of a DO block, so no inner semicolons).
ALTER TABLE "api_keys"    DROP CONSTRAINT IF EXISTS "api_keys_user_id_users_id_fk";
ALTER TABLE "api_keys"    ADD  CONSTRAINT "api_keys_user_id_users_id_fk" FOREIGN KEY ("user_id") REFERENCES "public"."users"("id") ON DELETE cascade ON UPDATE no action;
ALTER TABLE "balance_ops" DROP CONSTRAINT IF EXISTS "balance_ops_account_id_trading_accounts_id_fk";
ALTER TABLE "balance_ops" ADD  CONSTRAINT "balance_ops_account_id_trading_accounts_id_fk" FOREIGN KEY ("account_id") REFERENCES "public"."trading_accounts"("id") ON DELETE cascade ON UPDATE no action;
